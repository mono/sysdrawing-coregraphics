				...
				if (ds.LineStyle.IsVisible == true) {
					...
					if (ds.LineStyle.PlotMethod == LineStyle.PlotLinesMethodEnum.Lines) {
						for (int i = 1; i < ds.PointList.Count; i++) {
							if (!ds.IsY2Data) {
								g.DrawLine(aPen, cs.Point2D((CGPoint)ds.PointList[i - 1]),
												cs.Point2D((CGPoint)ds.PointList[i]));
							} else {
								g.DrawLine(aPen, cs.Point2DY2((CGPoint)ds.PointList[i - 1]),
												cs.Point2DY2((CGPoint)ds.PointList[i]));
							}
						}
