	...
	public class ColorMap {
		...
		public int[,] Spring ()
		{
			...
			for (int i = 0; i < colormapLength; i++) {
				spring[i] = 1f * i / (colormapLength - 1);
				cmap[i, 0] = alphaValue;
				cmap[i, 1] = 255;
		...
		public int[,] Summer ()
		{
			...
			for (int i = 0; i < colormapLength; i++) {
				summer[i] = 1f * i / (colormapLength - 1);
				cmap[i, 0] = alphaValue;
				cmap[i, 1] = (int)(255 * summer[i]);
				cmap[i, 2] = (int)(255 * .5f * (1 + summer[i]));
				cmap[i, 3] = (int)(255 * .4f);
			}
			return cmap;
		}
		...
		public int[,] Autumn ()
		{
			...
			for (int i = 0; i < colormapLength; i++)
			{
				autumn[i] = 1f * i / (colormapLength - 1);
				cmap[i, 0] = alphaValue;
				cmap[i, 1] = 255;
		...
		public int[,] Winter ()
		{
			...
			for (int i = 0; i < colormapLength; i++) {
				winter[i] = 1f * i / (colormapLength - 1);
				cmap[i, 0] = alphaValue;
				cmap[i, 1] = 0;
				cmap[i, 2] = (int)(255 * winter[i]);
				cmap[i, 3] = (int)(255 * (1f - 0.5f * winter[i]));
			}
			return cmap;
		}
		...
		public int[,] Jet ()
		{
			...
			var n = (int)Math.Ceiling (colormapLength / 4f);
			int nMod = 0;
			...
			for (int i = 0; i < fArray.Length; i++) {
				...
				green[i] = (int)Math.Ceiling(n / 2f) - nMod + i;
				red[i] = green[i] + n;
				blue[i] = green[i] - n;
			}
		...
		public int[,] Hot ()
		{
			...
			for (int i = 0; i < colormapLength; i++) {
				...
				if (i < 2 * n)
					...
					blue[i] = 1f * (i + 1 - 2 * n) / (colormapLength - 2 * n);
				cmap[i, 0] = alphaValue;
